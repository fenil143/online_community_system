...
const getPostsComments = async (postId) => {
  try {
    const postResponse = await axios.get(`http://localhost:8000/getPostById/${postId}`);
    ...
    const commentsDetails = await Promise.all(
      comments.map(async (comment) => {
        const commentResponse = await axios.get(`http://localhost:8000/getCommentById/${comment}`);
